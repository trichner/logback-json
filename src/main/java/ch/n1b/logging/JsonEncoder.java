...
public class JsonEncoder extends EncoderBase<ILoggingEvent> {
  ...
  public byte[] encode(ILoggingEvent event) {
    ...
    try {
      var generator = new JsonGenerator(baos);
      ...
      generator.writeRaw('\n');
    } catch (NullPointerException | JsonGenerator.WriteException e) {
  ...
  private void writeServiceContext(JsonGenerator generator) {
  ...
  private void writeLogger(JsonGenerator generator, ILoggingEvent logRecord) {
    generator.writeStringField("logger", logRecord.getLoggerName());
  }

  private void writeMessage(JsonGenerator generator, ILoggingEvent logRecord) {
    generator.writeStringField("message", logRecord.getFormattedMessage());
  }

  private void writeSeverity(JsonGenerator generator, ILoggingEvent logRecord) {
    // should we map that to the GCP levels?
    generator.writeStringField("severity", logRecord.getLevel().toString());
  }

  private void writeTimestamp(JsonGenerator generator, ILoggingEvent logRecord) {
    generator.writeStringField("time", timeFormatter.format(logRecord.getInstant()));
  }

  private void writeThread(JsonGenerator generator, ILoggingEvent logRecord) {
    generator.writeStringField("thread_name", logRecord.getThreadName());
  }

  private void writeError(
      JsonGenerator generator, ILoggingEvent logRecord, Map<String, String> mdc) {
  ...
  private void writeMdc(JsonGenerator generator, Map<String, String> mdc) {
    if (mdc.isEmpty()) {
      return;
    }
  ...
  private void writeKeyValue(JsonGenerator generator, ILoggingEvent event) {
  ...
  private void writeValue(JsonGenerator generator, Map<String, Object> m) {
  ...
  private void writeStackTrace(JsonGenerator generator, ILoggingEvent event) {
  ...
  private static String escapeJson(String s) {
    var baos = new ByteArrayOutputStream((int) (s.length() * 1.5));
    try {
